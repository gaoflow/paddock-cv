...
完整岗位族、职责边界和一级来源见 [`docs/research/f1-role-taxonomy-strategy-2026-07-16.md`](research/f1-role-taxonomy-strategy-2026-07-16.md)。网站使用的机器可读版本是 `data/f1_role_taxonomy.json`。
...
本轮确实使用了 MySearch，不是只写在方法论里。本机调用路径为 `http://127.0.0.1:9874/exa/search`，局域网地址当前为 `http://192.168.1.36:9874`。同机服务固定使用 loopback，避免后续 DHCP 地址变化再次中断。MySearch 用于：
...
本轮实质成果存于 `data/f1_public_engineering_people_expansion.json`：补入此前因 title alias 和岗位族缺失而漏掉的策略、比赛工程、车辆性能、仿真、控制电子、轮胎、赛道气动和可靠性人员；所有新增记录均保留英文原始职位。
