...
真实数据集（约 480 位在职人员的聚合履历与缓存照片）出于隐私与版权考虑
不随仓库分发；`data/sample/` 附带一套结构完全同构、明显虚构的示例数据
...
面板的布局体系和搜索实现已整理成可复用的 agent skill：
[`skills/paddock-dashboard/SKILL.md`](skills/paddock-dashboard/SKILL.md)。
放进你的 agent skills 目录即可复刻（或扩展）Paddock CV 风格的名册
UI —— Apple 深色 token、车队卡片、人员行、hash 路由 CV 抽屉，以及单输入实时搜索。
